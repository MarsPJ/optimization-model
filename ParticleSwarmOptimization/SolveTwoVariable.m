%% 粒子群算法PSO: 求解函数Obj_funTwoVar  y = x1^2+x2^2-x1*x2-10*x1-4*x2+60在[-15,15]内的最小值(动画演示）
clear; clc
...
for i = 1:n  % 循环整个粒子群，计算每一个粒子的适应度
    fit(i) = Obj_funTwoVar(x(i,:));   % 调用Obj_funTwoVar函数来计算适应度
end 
...
for d = 1:K  % 开始迭代，一共迭代K次
    for i = 1:n   % 依次更新第i个粒子的速度与位置
        ...
        fit(i) = Obj_funTwoVar(x(i,:));  % 重新计算第i个粒子的适应度
        if fit(i) < Obj_funTwoVar(pbest(i,:))   % 如果第i个粒子的适应度小于这个粒子迄今为止找到的最佳位置对应的适应度
           pbest(i,:) = x(i,:);   % 那就更新第i个粒子迄今为止找到的最佳位置
        end
        if  fit(i) < Obj_funTwoVar(gbest)  % 如果第i个粒子的适应度小于所有的粒子迄今为止找到的最佳位置对应的适应度
            gbest = pbest(i,:);   % 那就更新所有粒子迄今为止找到的最佳位置
        end
    end
    fitnessbest(d) = Obj_funTwoVar(gbest);  % 更新第d次迭代得到的最佳的适应度
    pause(0.1)  % 暂停0.1s
...
disp('此时最优值是：'); disp(Obj_funTwoVar(gbest))